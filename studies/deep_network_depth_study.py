...
def negative_control(n_seeds=3):
    ...
    depths = [1, 2, 3, 4, 5, 6]
    curve = measure_depth_curve(
        lambda L, sd: (1 - _fit_mlp(X, y, L, sd), _fit_mlp(X, y, L, sd)), depths, list(range(n_seeds))
    )
    print("  val acc by depth: " + "  ".join(f"L{d}={curve.acc_mean[i]:.3f}" for i, d in enumerate(curve.depths)))
...
def esol_production_depth(max_depth_cap=8):
    ...
    print(f"  selected  width*={detail['width_star']}  depth*={detail['depth_star']}  area*={detail['area_star']}")
    print(
        f"  boundary extension fired: {detail['depth_extended']}  "
...
def deep_generation_is_not_deep_requirement(n_seeds=3):
    ...
    for D in (4, 6):
        ...
        depths = [1, 2, 3, 4, 5, 6]
        curve = measure_depth_curve(
            lambda L, sd, X=X, y=y: (1 - _fit_mlp(X, y, L, sd), _fit_mlp(X, y, L, sd)), depths, list(range(n_seeds))
        )
