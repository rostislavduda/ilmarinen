...
class EquivariantMLP:
    ...
    def __init__(self, gens, n_in_vec, vec_dim, hidden_vec=4, hidden_scalar=8, depth=2, n_out=1, metric=None):
        self.gens = gens
    ...
    def torch_module(self):
        ...
        class _Net(nn.Module):
            def __init__(m):
                ...
                m.lins = nn.ModuleList(
                    [EquivariantLinear(outer.hid_rep, outer.hid_rep).torch_module() for _ in range(outer.depth)]
                )
                ...
                m.readout = nn.Sequential(
                    nn.Linear(outer.hidden_scalar + n_inv, 32), nn.Tanh(), nn.Linear(32, outer.n_out)
                )
                m.vdim = vdim
