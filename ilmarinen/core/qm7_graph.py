...
def _build_qm7(path, cutoff_bohr, n_max, with_pos):
    ...
    for i in range(N):
        ...
        if len(srcs) == 0:                                # fallback: connect nearest neighbor
            dsts = D.argmin(1); srcs = np.arange(n)
        g = {"x": torch.tensor(feat),
             "edge_index": torch.tensor(np.stack([srcs, dsts], axis=0).astype(np.int64))}
        if with_pos:
...
def collate_equivariant(graphs, idx):
    ...
    for g_i, i in enumerate(idx):
        ...
        xs.append(g["x"]); ps.append(g["pos"]); eis.append(g["edge_index"] + offset)
        batch.append(torch.full((n,), g_i, dtype=torch.long)); offset += n
    return (torch.cat(xs, 0), torch.cat(ps, 0), torch.cat(eis, 1),
            torch.cat(batch, 0), len(idx))
