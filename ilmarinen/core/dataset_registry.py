...
def load_bloodmnist(reduced, device):
    ...
    return {
        "train": AllData.dense_tensor(zc(d["train_x"])[:ntr], y=d["train_y"][:ntr]),
...
def load_mnist(reduced, device):
    ...
    return {
        "train": AllData.dense_tensor(Xtr, y=ytr),
...
def load_organmnist3d(reduced, device):
    ...
    def zn(x):
        return torch.tensor((x - x.mean()) / x.std()).unsqueeze(1)

    ntr = qscale(400) if reduced else len(d["train_y"])
    return {
        "train": AllData.dense_tensor(zn(d["train_x"])[:ntr], y=d["train_y"][:ntr]),
...
def load_esol(reduced, device):
    ...
    return {
        "train": AllData.graphs([nf[i] for i in tr], [ed[i] for i in tr], y=(y[tr] - ym) / ys),
        "test": AllData.graphs([nf[i] for i in te], [ed[i] for i in te], y=(y[te] - ym) / ys),
        "task": "regression",
        "chance": 0.0,
        "field": "chemistry (solubility)",
        "target_scale": float(ys),
        "target_units": "log mol/L",  # de-normalize z-scored logS -> physical-unit MAE
...
def load_rmd17(reduced, device):
    ...
    ys = float(Eraw.std())  # retain the kcal/mol scale for physical MAE
    E = (Eraw - Eraw.mean()) / ys
    Z = data["Z"]
...
def load_qm7(reduced, device):
    ...
    nf, ed, pos, y = _mol_graphs_from_qm7(
        n_max=qscale(1500) if reduced else None
    )  # standard: all 7165; quick: 1500 x scale
    ym, ys = y.mean(), y.std()
    tr, te = _split(len(nf))
    return {
        "train": AllData.graphs(
            [nf[i] for i in tr], [ed[i] for i in tr], y=(y[tr] - ym) / ys, positions=[pos[i] for i in tr]
        ),
        "test": AllData.graphs(
            [nf[i] for i in te], [ed[i] for i in te], y=(y[te] - ym) / ys, positions=[pos[i] for i in te]
        ),
        "task": "regression",
        "chance": 0.0,
        "field": "quantum chemistry",
        "target_scale": float(ys),
        "target_units": "kcal/mol",  # de-normalize z-scored MAE -> physical
        "sota": "MAE < 1 kcal/mol = chemical accuracy (SchNet/PaiNN); R2 ~0.999+",
        "rotated": True,
    }
...
def load_qm9(reduced, device):
    ...
    return {
        ...
        "chance": 0.0,
        "field": "quantum chemistry",
        "target_scale": float(ys * 27211.386),
        "target_units": "meV",  # U0 std (Hartree) -> meV MAE
        "sota": "U0 MAE ~5-15 meV (SchNet/PaiNN/DimeNet); R2 ~0.999+",
        "rotated": True,
    }
...
def load_jetnet(reduced, device):
    ...
    return {
        "train": AllData.point_sets([nf[i] for i in tr], y=labels[tr]),
...
def load_darcy2d(reduced, device):
    ...
    N = 20
    n = 130 if not reduced else qscale(90)

    def solve(a, f=1.0):
        h = 1.0 / (N - 1)
        Ni = N - 2
        idx = lambda i, j: i * Ni + j
        A = lil_matrix((Ni * Ni, Ni * Ni))
        b = np.full(Ni * Ni, f * h * h)
        for i in range(Ni):
            for j in range(Ni):
                I, J = i + 1, j + 1
                c = idx(i, j)
                aE = 0.5 * (a[I, J] + a[I, J + 1])
                aW = 0.5 * (a[I, J] + a[I, J - 1])
                aN = 0.5 * (a[I, J] + a[I + 1, J])
                aS = 0.5 * (a[I, J] + a[I - 1, J])
                A[c, c] = aE + aW + aN + aS
                if j + 1 < Ni:
                    A[c, idx(i, j + 1)] = -aE
                if j - 1 >= 0:
                ...
                if i - 1 >= 0:
                    A[c, idx(i - 1, j)] = -aS
        u = np.zeros((N, N))
        u[1:-1, 1:-1] = spsolve(csr_matrix(A), b).reshape(Ni, Ni)
    ...
    for _ in range(n):
        ...
        for i in range(4):
            for j in range(4):
                field += c[i, j] * np.cos(i * np.pi * X) * np.cos(j * np.pi * Y)
        a = np.exp(0.5 * field).astype(np.float32)
        aa.append(a)
        uu.append(solve(a))
    aa = np.array(aa, np.float32)
    uu = np.array(uu, np.float32)
    tr, te = _split(n)
    return {
        "train": AllData.functions(a=aa[tr], y=uu[tr]),
        "test": AllData.functions(a=aa[te], y=uu[te]),
        "task": "regression",
        "chance": 0.0,
        "field": "PDE (2D Darcy flow, elliptic)",
        "sota": "field R2 ~0.999 (FNO)",
    }
