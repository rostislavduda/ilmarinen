...
class _PersistenceMixin:
            ...
            for j in range(0, len(X), bs):
                with torch.no_grad():
                    if mod == "sequence":
                        ...
                        o = (
                            net.forward(xb)
                            if self._infer_readout == "flatten"
        ...
        for j in range(0, n, 128):
            ...
            with torch.no_grad():
                if mod == "set":
                    ...
                    outs.append(net(x, ei, b, ng).cpu())
                else:
                    outs.append(net(x, p, ei, b, ng).cpu())
        return torch.cat(outs)
    ...
    def predict(self, data):
        ...
        if self._infer_task == "classification":
            return (
                out.argmax(1).numpy() if out.dim() == 2 and out.shape[1] > 1 else (out.squeeze(-1) > 0).long().numpy()
            )
    ...
    def predict_proba(self, data):
        ...
        if self._infer_task != "classification":
            raise ValueError(
                "predict_proba is only defined for a classification model "
    ...
    def save(self, path=None, dirpath=None, stem=None):
        ...
        payload = {
            ...
            "config": {
                ...
                for k in ("width", "depth", "epochs", "lr", "seed", "select", "sparsity_mu", "gibbs_beta")
            },
