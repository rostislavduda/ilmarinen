...
def _build_library(d):
    ...
    terms = {
        "1": np.ones_like(d),
        "d": d, "d^2": d ** 2, "d^3": d ** 3,
        "1/d": 1.0 / (d + eps), "1/d^2": 1.0 / (d ** 2 + eps),
        "1/d^6": 1.0 / (d ** 6 + eps), "1/d^12": 1.0 / (d ** 12 + eps),
        "exp(-d)": np.exp(-d), "exp(-d^2)": np.exp(-d ** 2),
        "sin(d)": np.sin(d), "cos(d)": np.cos(d), "log(d)": np.log(d + 1.0 + eps),
    }
...
def _fit_k_terms(A, names, phi, k):
    ...
    An = A / scale
    resid = phi.copy(); chosen = []; c = np.zeros(0)
    for _ in range(k):
        corr = np.abs(An.T @ resid)
        if chosen:
            corr[chosen] = -1.0
        j = int(np.argmax(corr)); chosen.append(j)
...
def symbolic_readout(d, phi, r2_accept=0.999, max_terms=3, min_gain=0.02):
    ...
    if clean:
        ...
        verdict = (f"NO clean symbolic form (best R^2={r2:.4f} < {r2_accept}); "
                   f"defer to the exact spline -- this expression is only an approximate lens")
    return {
        "expression": expr, "coefs": coefs, "r2": r2, "max_abs_error": max_err,
        "n_terms": len(coefs), "clean": clean, "verdict": verdict,
        ...
        for core in getattr(c, "cores", []):
            ...
            if name and "kan" in name and hasattr(core, "radial_function"):
                try:
                    ...
                    continue
                d = np.asarray(d); phi = np.asarray(phi)
                ...
                for ch, curve in channels:
                    ...
                    rep["cell"] = ci; rep["primitive"] = name; rep["channel"] = ch
                    out.append(rep)
    return out
