...
class _Conv4dCore(nn.Module):
    def __init__(self, cin, width, kt=3, ks=3, depthwise=False, kt1=False):
        ...
        self.weight = nn.Parameter(
            torch.randn(cout, cin // groups, self.kt, ks, ks, ks) * (2.0 / (cin * self.kt * ks**3)) ** 0.5
        )
...
class _Pointwise4d(nn.Module):
    def __init__(self, cin, width):
        super().__init__()
        self.lin = nn.Conv3d(cin, width, 1)  # 1x1x1 over space applied per-time
...
class _Grid4dCell(nn.Module):
    def __init__(self, cin, width, primitives, grid_shape):
        ...
        for p in self.primitives:
            if p == "conv4d":
                cores.append(_Conv4dCore(cin, width, kt=3, ks=3))
            elif p == "conv4d_kt1":
                cores.append(_Conv4dCore(cin, width, kt=3, ks=3, kt1=True))
            elif p == "conv_dw":
                cores.append(_Conv4dCore(cin, width, kt=3, ks=3, depthwise=True))
            elif p == "pointwise":
                cores.append(_Pointwise4d(cin, width))
            elif p == "dense":
                cores.append(_Dense4d(cin, width, grid_shape))
            elif p == "norm":
                cores.append(_Norm4d(cin, width))
            else:
                raise ValueError(f"unknown 4D primitive {p}")
        ...
        self.fixers = nn.ModuleList(
            [(nn.Conv3d(cin, width, 1) if p == "conv_dw" and cin != width else None) for p in self.primitives]
        )
...
class Grid4dSchema(nn.Module):
    def __init__(
        ...
        for _ in range(depth):
            self.cells.append(_Grid4dCell(c, width, primitives, grid_shape))
            c = width
