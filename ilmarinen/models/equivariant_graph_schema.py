...
class _KANMessage(nn.Module):
    ...
    def _spline(self, dist):
        ...
        x0 = self.knots[idx - 1]; x1 = self.knots[idx]
...
class _PaiNNMessage(nn.Module):
    ...
    def __init__(self, c0, c1, n_rbf=8, rbf_cutoff=3.5):
        ...
        self.rbf = nn.Linear(n_rbf, c0 + c1 + c1); _init_lin(self.rbf)
        self.register_buffer("centers", torch.linspace(0.0, rbf_cutoff, n_rbf))
        spacing = rbf_cutoff / max(n_rbf - 1, 1)
...
class _EquivCell(nn.Module):
    ...
    def forward(self, s, v, pos, edge_index):
        ...
        for c in self.cores:
            ...
            s_outs.append(so); v_outs.append(vo)
...
class EquivariantGraphSchema(nn.Module):
    ...
    def __init__(self, fin, c0=16, c1=8, depth=3, n_out=1, seed=0,
        ...
        self.embed = nn.Linear(fin, c0); _init_lin(self.embed)       # scalars in; vectors start at 0
        self.cells = nn.ModuleList([_EquivCell(c0, c1, self.primitives) for _ in range(depth)])
        self.head = nn.Linear(c0, n_out); _init_lin(self.head)       # invariant head (type-0 only)
...
def build_equivariant_graph_schema(n_in=None, c0=16, c1=8, depth=3, n_out=1, seed=0,
    ...
    return EquivariantGraphSchema(fin=n_in, c0=c0, c1=c1, depth=depth, n_out=n_out,
