...
class _DilatedConv2dSpatial(nn.Module):
    ...
    def __init__(self, cin, width, hw, kernel_size=3, dilations=(1, 2, 4, 8)):
        ...
        self.branches = nn.ModuleList(
            [
                nn.Conv2d(
                    cin, bw, kernel_size=kernel_size, dilation=d, padding=((kernel_size - 1) // 2) * d
                )  # 'same' padding per dilation
    ...
    def __init__(
        ...
        for m in self.stem:
            if isinstance(m, nn.Conv2d):
                _init_conv(m)
        self.cells = nn.ModuleList([_SpatialCell(width, width, hw, self.primitives) for _ in range(depth)])
...
def build_spatial_schema(
    ...
    return SpatialSchema(
        width=width, hw=hw, depth=depth, n_in=n_in, n_classes=n_out, seed=seed, primitives=primitives, img_size=img_size
    )
