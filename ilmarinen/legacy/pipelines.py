...
# ----------------------------------------------------------------------
def validate_width_sparsity(data: FashionMNIST, cls_a=0, cls_b=6, lambdas=(0.10, 0.06, 0.03), verbose=True):
    ...
    for lam in lambdas:
        ...
        if verbose:
            print(
                f"  lambda={lam:4.2f}: K={res.K:3d} neurons  "
...
def validate_criticality(
    ...
    if verbose:
        ...
        for name, pr in phases.items():
            ...
            print(f"  {name:8s} sw2={inits[name]:<5} chi1={pr.chi1:.3f} c*={pr.c_star:.3f} xi={xi}")
...
def validate_priced_depth(
    ...
    sigma_w2=1.76,
    epochs=15,
    prices=(0.05, 0.02, 0.01, 0.005, 0.002),
    verbose=True,
):
    ...
    def train_eval_fn(depth, seed):
        model = build_model(model_kind, depth=depth, width=width, sigma_w2=sigma_w2, seed=seed)
    ...
    if verbose:
        ...
        for i, L in enumerate(curve.depths):
            print(f"    L={L:3d}: {curve.S_mean[i]:.4f} +- {curve.S_se[i]:.4f}  acc={curve.acc_mean[i]:.3f}")
...
# ----------------------------------------------------------------------
def validate_sequential_baseline(
    data: FashionMNIST,
    per_class_tr=300,
    per_class_te=100,
    width=128,
    ...
    Xte = torch.tensor(Xte_np)
    yte = torch.tensor(yte_np)

    results = {}
    if verbose:
        print(f"  sequential Fashion-MNIST T=784: train {tuple(Xtr.shape)}, chance=0.100, achievable(gated)~0.90")
    for name, sw2 in inits:
        ...
        results[name] = {"sigma_w2": sw2, "chi1": pr.chi1, "xi_t": pr.xi, "val_acc": va, "val_loss": vl}
        if verbose:
...
def validate_supergraph(
    ...
    if verbose:
        print(
            f"  soft search: acc={acc_soft:.3f}, alpha=[plain {alpha[0]:.3f}, gated {alpha[1]:.3f}], selected={choice}"
        )
    ...
    return {"acc_soft": acc_soft, "acc_warm": acc_warm, "acc_disc": acc_disc, "alpha": alpha, "selected": choice}
...
def _make_copy(n, K=4, delay=30, S=6, seed=0):
    ...
    for i in range(n):
        s = rng.integers(1, S + 1, size=K)
        Y[i] = s - 1
        for t in range(K):
            X[i, t, s[t]] = 1.0
        for t in range(K, K + delay):
            X[i, t, 0] = 1.0
        for t in range(K + delay, L):
            X[i, t, S + 1] = 1.0
    return torch.tensor(X), torch.tensor(Y), V


def validate_supergraph_copy(depth=1, width=96, K=4, delay=30, S=6, epochs=22, verbose=True):
    ...
    Xfull = torch.cat([Xw, Xa])
    yfull = torch.cat([yw, ya])
    ...
    ow = torch.optim.Adam(wp, lr=0.005)
    ...
    for _ in range(epochs):
        ...
        for i in range(0, nw, bs):
            abi = pa[aptr : aptr + bs]
            aptr = (aptr + bs) % max(na - bs, 1)
            if len(abi) > 0:
                oa.zero_grad()
                o = net.forward_seq_readout(Xa[abi], K)
            ...
            wbi = pw[i : i + bs]
            ow.zero_grad()
            o = net.forward_seq_readout(Xw[wbi], K)
    ...
    opt = torch.optim.Adam(d.parameters(), lr=0.003)
    n = len(Xfull)
    for _ in range(12):
        perm = torch.randperm(n)
        for i in range(0, n, bs):
            bi = perm[i : i + bs]
            opt.zero_grad()
            o = d.forward_seq_readout(Xfull[bi], K)
            l = lf(o.reshape(-1, S), yfull[bi].reshape(-1))
            if not torch.isfinite(l):
                break
            l.backward()
    ...
    if verbose:
        print(f"  copy task depth={depth}: soft={acc_soft:.3f}, choice={d.choice}, peak_alpha_gated={peak[1]:.3f}")
        print(f"    discretize {acc_raw:.3f} -> finetune {acc_ft:.3f}")
    return {
        "soft": acc_soft,
        "choice": d.choice,
        "peak_alpha_gated": float(peak[1]),
        "discretized_raw": acc_raw,
        "discretized_finetuned": acc_ft,
    }
