...
def _roc_auc(out, y):
    ...
    per_class = [
        _auc(probs[:, c], (y == c).astype(int)) for c in range(out.shape[1]) if 0 < int((y == c).sum()) < len(y)
    ]
...
def _eval_test(
    ...
    if mod == "operator":
        ...
        a = (
            test.dense
            if isinstance(test.dense, torch.Tensor)
            else torch.tensor(np.asarray(test.dense), dtype=torch.float32)
        )
        xg = (
            test.grid
            if isinstance(test.grid, torch.Tensor)
            else torch.tensor(np.asarray(test.grid), dtype=torch.float32)
        )
        with torch.no_grad():
    ...
    if mod == "generated_equivariant":
        ...
        if mod == "sequence" and X.dim() == 2:
            X = X.unsqueeze(-1)
        if mod == "spatial" and X.dim() == 3:
            X = X.unsqueeze(1)
        if mod == "volumetric" and X.dim() == 4:
            X = X.unsqueeze(1)
        ...
        outs = []
        n = len(test.node_feats)
        for j in range(0, n, relational_bs):
            ids = np.arange(j, min(j + relational_bs, n))
            xs, eis, ps, batch = [], [], [], []
            off = 0
            for gi, i in enumerate(ids):
                nf = test.node_feats[i]
                k = nf.shape[0]
                xs.append(torch.as_tensor(nf, dtype=torch.float32))
                batch.append(torch.full((k,), gi, dtype=torch.long))
                if mod == "equivariant":
                    p = torch.as_tensor(test.positions[i], dtype=torch.float32)
                    ps.append(p @ R.T if R is not None else p)
                if mod in ("graph", "equivariant"):
                    eis.append(torch.as_tensor(test.edges[i], dtype=torch.long) + off)
                off += k
            x = torch.cat(xs).to(dev)
            b = torch.cat(batch).to(dev)
            ng = len(ids)
            with torch.no_grad():
                if mod == "set":
                    outs.append(net(x, b, ng).cpu())
                elif mod == "graph":
                    outs.append(net(x, torch.cat(eis, 1).to(dev), b, ng).cpu())
                else:
                    outs.append(net(x, torch.cat(ps).to(dev), torch.cat(eis, 1).to(dev), b, ng).cpu())
        out = torch.cat(outs)
...
def add_pipeline_args(ap):
    ...
    ap.add_argument(
        "--equivariant_realization",
        default="emlp",
        choices=["emlp", "scalable"],
    ...
    ap.add_argument(
        "--auto_epoch_min_delta",
        type=float,
        default=0.01,
        help="--auto_epoch: minimum RELATIVE train-loss reduction that counts as improvement (default 0.01 = 1%%)",
    )
    ap.add_argument(
        "--auto_epoch_min_epochs",
        type=int,
        default=5,
...
def make_allgraph(args, bud, device, router, tzmu, enabled_sg):
    ...
    return AllGraph(
        width=bud["width"],
        depth=bud["depth"],
        epochs=max(1, int(bud["epochs"] * args.epochs_scale)),
        device=device,
        verbose=False,
        seed=0,
        sparsity_mu=args.sparsity_mu,
        tensorize_mu=tzmu,
        contract_router=router,
        symmetry_routing=args.symmetry_routing,
        canonicalize_reuse=args.canonicalize,
        discover_equivariant_contract=(
            True if args.discover == "menu" else "extended" if args.discover == "extended" else False
        ),
        kernel_from_xi=args.kernel_from_xi,
        angular_from_data=args.angular_from_data,
        nonlinear_symmetry_fallback=args.nonlinear_symmetry_fallback,
        enabled_contracts=enabled_sg,
        contract_posterior=args.contract_posterior,
        report_llc=args.report_llc,
        deploy_nonlinear_contract=args.deploy_nonlinear_contract,
        equivariant_realization=args.equivariant_realization,
        price_equivariance=args.price_equivariance,
        price_modes=args.price_modes,
        developmental_llc=args.developmental_llc,
        report_thermo=args.report_thermo,
        report_response=args.report_response,
        report_ledger=args.report_ledger,
        price_singular=args.price_singular,
        singular_mu=args.singular_mu,
        singular_llc_steps=args.singular_llc_steps,
        singular_llc_chains=args.singular_llc_chains,
        progress=True,
...
def main():
    ...
    print(
        f"select={args.select}  sparsity_mu={args.sparsity_mu}  tensorize_mu={tzmu}  tiebreak={args.tiebreak}  learned_router={not args.no_learned_router}"
    )
    ...
    for _i in range(len(loaded)):
        ...
        try:
            ...
            print(
                f"[{mg.contract:11}] {name:16} {metric:9}={value:.4f}{extra_str} skill={skill:+.3f} "
