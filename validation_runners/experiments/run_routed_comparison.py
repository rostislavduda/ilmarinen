...
def metaoptimize(
    ...
    return {
        "depth": Lstar,
        "width": Kstar,
        "val": float(curve.acc_mean[list(depths).index(Lstar)]) if Lstar in depths else 0.0,
        "test": ta,
        "arch": net.architecture(),
        "params": sum(p.numel() for p in net.parameters()),
        "selection": "priced",
        "depth_marginals": [(round(m[0], 1), round(m[1], 4)) for m in curve.marginals],
    }
...
def run(args):
    ...
    sel = args.selection
    print(
        f"\n########## {args.dataset} (n_train={len(Xtr)}, dim={d}, maj={maj:.3f}) [selection={sel}] ##########",
        flush=True,
    )
    ...
    if args.symmetry:
        ...
        sym = discover_and_reduce(
            Xw,
            yw.float(),
            n_refits=2,
            epochs=args.sym_epochs,
            coordinate_structure="unknown",
            scale_aware=True,
            verbose=False,
        )
        found = (
            f"cont={[k for k, _ in sym['continuous']]}, z2={len(sym['z2'])}, "
    ...
    def build_seq(width, depth, seed):
        return build_schema(
            depth=depth, width=width, n_in=1, n_out=n_out, seed=seed, primitives=SEQ_PRIMS, readout=args.readout
        )
    ...
    print(
        f"[FIXED ] seq repr: arch={F['arch']} d{F['depth']} w{F['width']} "
        f"params={F['params']} TEST={F['test']:.3f}{mtag}",
        flush=True,
    )
    ...
    print(
        f"[ROUTED] detected structure={route['structure']} shape={route['shape']} -> {route['kind']} schema", flush=True
    )
    ...
    print(
        f"[ROUTED] arch={R['arch']} d{R['depth']} w{R['width']} params={R['params']} TEST={R['test']:.3f}{mtag}",
        flush=True,
    )
    print(f"[DELTA ] FIXED {F['test']:.3f} -> ROUTED {R['test']:.3f} ({R['test'] - F['test']:+.3f})", flush=True)
