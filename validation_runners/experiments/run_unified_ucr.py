...
def run(args):
    ...
    for l, pk in enumerate(peak):
        print(f"    layer {l} peak-alpha: " + ", ".join(f"{p}={v:.2f}" for p, v in zip(prims, pk)))
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--primitives", default=",".join(DEFAULT_PRIMS), help="comma-separated subset of: " + ",".join(DEFAULT_PRIMS)
    )
