...
def make_task(task, n=400, H=20, seed=0):
    ...
    for i in range(n):
        ...
        cx = rng.randint(2, H - 2 - sep)
        img = np.zeros((H, H))
        img[cy, cx] = 1.0
        img[cy, cx + sep] = 1.0 if same else 0.4
        X[i, 0] = gaussian_filter(img, 0.8) + 0.3 * rng.randn(H, H)
        y[i] = 0 if same else 1
...
def run(args):
    ...
    print(f"correlation-length recommendation: xi={rec['xi']:.2f} -> {rec['primitive']} (kernel {rec['kernel_size']})")
    ...
    print(f"conv candidates: {conv_prims}  |  structural costs (k^2, norm): {[round(float(c), 2) for c in costs]}")
    ...
    def solo(prim, width=16, epochs=args.epochs):
        ...
        net = build_spatial_schema(
            width=width, hw=10, depth=1, n_in=1, n_classes=2, primitives=(prim,), img_size=Xtr.shape[-1]
        )
    ...
    print(f"\nsolo accuracy per receptive field: { {p: round(a, 3) for p, a in zip(conv_prims, accs)} }")
    ...
    print(
        "As mu rises, the priced action trades receptive field for cheapness -- k7 -> k5 -> k3 -- "
