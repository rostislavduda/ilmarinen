...
def metaoptimize(
    Xw, yw, Xv, yv, Xte, yte, n_in, n_out, prims, widths, depths, seed, epochs, readout, mu, gamma, acc_tol
):
    rows = []
    for depth in depths:
        for w in widths:
            va, net = fit(Xw, yw, Xv, yv, n_in, n_out, prims, w, depth, seed, epochs, readout, mu, gamma)
            rows.append((depth, w, va, net))
    best_va = max(va for _, _, va, _ in rows)
    rows_sorted = sorted(rows, key=lambda r: (r[0], r[1]))
    depth_s, w_s, va_s, net_s = next((d, w, va, net) for (d, w, va, net) in rows_sorted if va >= best_va - acc_tol)
    with torch.no_grad():
    ...
    return {
        ...
        "val_acc": va_s,
        "test_acc": test_acc,
        "arch": net_s.architecture(),
        "params": sum(p.numel() for p in net_s.parameters()),
...
def run(args):
    ...
    print(
        f"[RAW] arch={A['arch']} depth={A['depth']} width={A['width']} "
        f"params={A['params']} val={A['val_acc']:.3f} TEST={A['test_acc']:.3f}",
        flush=True,
    )
    ...
    syms = (
        f"continuous={[k for k, _ in sym['continuous']]}, cyclic={sym['cyclic']}, "
    ...
    print(
        f"[SYM] arch={B['arch']} depth={B['depth']} width={B['width']} "
        f"params={B['params']} val={B['val_acc']:.3f} TEST={B['test_acc']:.3f} "
        f"(feature dim {Xw_flat.shape[1]}->{n_in_r})",
        flush=True,
    )
    print(
        f"[DELTA] raw TEST {A['test_acc']:.3f} vs sym TEST {B['test_acc']:.3f} ({B['test_acc'] - A['test_acc']:+.3f})",
        flush=True,
    )
