...
class GRUNet(nn.Module):
    def __init__(s, chan, n_out, w):
        super().__init__()
        s.rnn = nn.GRU(chan, w, batch_first=True)
        s.head = nn.Linear(w, n_out)

    def forward(s, x):
        o, _ = s.rnn(x)
        return s.head(o.mean(1))


class CNNNet(nn.Module):
    def __init__(s, chan, n_out, w):
        super().__init__()
        s.c1 = nn.Conv1d(chan, w, 5, padding=2)
        s.c2 = nn.Conv1d(w, w, 5, padding=2)
        s.head = nn.Linear(w, n_out)

    def forward(s, x):
        h = torch.relu(s.c1(x.transpose(1, 2)))
        h = torch.relu(s.c2(h))
        return s.head(h.mean(2))


class MLPNet(nn.Module):
    def __init__(s, chan, T, n_out, w):
        super().__init__()
        s.f = nn.Linear(chan * T, w)
        s.h = nn.Linear(w, n_out)
...
def run(args):
    ...
    results = {}
    for name, ctor in [
        ("GRU", lambda: GRUNet(chan, n_out, args.width)),
