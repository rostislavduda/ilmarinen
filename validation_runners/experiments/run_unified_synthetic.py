...
def make_copy(n, S, delay, V, seed):
    rng = np.random.default_rng(seed); T = S + delay + S
    X = np.zeros((n, T, V + 2), np.float32); Y = np.zeros((n, S), np.int64)
    for i in range(n):
...
def make_adding(n, T, seed):
    ...
    vals = rng.uniform(0, 1, (n, T)).astype(np.float32); mk = np.zeros((n, T), np.float32)
    Y = np.zeros(n, np.float32)
    for i in range(n):
        idx = rng.choice(T, 2, replace=False); mk[i, idx] = 1.0; Y[i] = vals[i, idx].sum()
    return torch.tensor(np.stack([vals, mk], -1)), torch.tensor(Y)


def make_recall(n, N, Vk, Vv, seed):
    rng = np.random.default_rng(seed); T = N + 1
    X = np.zeros((n, T, Vk + Vv), np.float32); Y = np.zeros(n, np.int64)
    for i in range(n):
        keys = rng.permutation(Vk)[:N]; vals = rng.integers(0, Vv, N)
        for s in range(N):
            X[i, s, keys[s]] = 1.0; X[i, s, Vk + vals[s]] = 1.0
        qi = rng.integers(N); X[i, N, keys[qi]] = 1.0; Y[i] = vals[qi]
...
def train_select(net, X, Y, loss_fn, epochs, seqk=None):
    ...
    ow = torch.optim.Adam(wp, lr=0.003); oa = torch.optim.Adam(ap, lr=0.02); n = len(X); bs = 64
    for ep in range(epochs):
        perm = torch.randperm(n)
        for i in range(0, n, bs):
            bi = perm[i:i + bs]
            ow.zero_grad(); l = loss_fn(net, X[bi], Y[bi], seqk)
            if torch.isfinite(l):
                l.backward(); torch.nn.utils.clip_grad_norm_(wp, 5.0); ow.step()
            oa.zero_grad(); la = loss_fn(net, X[bi], Y[bi], seqk)
...
def run(args):
    ...
    if args.task == "copy":
        ...
        net = build_schema(depth=1, width=64, n_in=args.V + 2, n_out=args.V,
                                       seed=args.seed, primitives=ALL, readout="last")
        def loss(net, x, y, k):
            o = net.forward_seq_readout(x, k); return nn.functional.cross_entropy(
                o.reshape(-1, o.shape[-1]), y.reshape(-1))
        ...
        net = build_schema(depth=1, width=64, n_in=2, n_out=1, seed=args.seed,
                                       primitives=ALL, readout="last", chrono_tmax=args.T)
        def loss(net, x, y, k):
        ...
        net = build_schema(depth=1, width=64, n_in=args.Vk + args.Vv,
                                       n_out=args.Vv, seed=args.seed, primitives=ALL, readout="last")
        def loss(net, x, y, k):
...
if __name__ == "__main__":
    ...
    # copy
    ap.add_argument("--S", type=int, default=6); ap.add_argument("--delay", type=int, default=15)
    ap.add_argument("--V", type=int, default=6)
    # adding
    ap.add_argument("--T", type=int, default=80)
    # recall
    ap.add_argument("--N", type=int, default=8); ap.add_argument("--Vk", type=int, default=8)
