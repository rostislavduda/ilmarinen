...
def main():
    ...
    print("=" * 100)
    print(f"QUICK VALIDATION of AllGraph  |  device={device}  data_scale={args.data_scale}  (sample-downscaled "
          f"MIRROR of run_standard_validation -- reduced subsets + small budgets; rough smoke, NOT a benchmark)")
    print(f"select={args.select}  sparsity_mu={args.sparsity_mu}  tensorize_mu={tzmu}  tiebreak={args.tiebreak}  "
          f"learned_router={not args.no_learned_router}  flatten_grids={args.flatten_grids}")
    print(f"symmetry_routing={args.symmetry_routing}  canonicalize={args.canonicalize}  discover={args.discover}  "
    ...
    for _i in range(len(loaded)):
        ...
        try:
            ...
            metric, value, extra = _eval_test(mg, d["test"], d["task"], rotated=d.get("rotated", False),
                                              ...
                                              bg_rejection=d.get("bg_rejection"), dense_bs=128, relational_bs=64,
                                              target_scale=d.get("target_scale"), target_units=d.get("target_units"))
            chance = d["chance"]
            extra_d = dict(extra)
            if d.get("auc"): skill = value
            elif d["task"] == "classification": skill = (value - chance) / (1 - chance)
            else: skill = extra_d.get("R2", value)          # R2 stays the skill axis even when MAE is the headline
            dt = time.time() - t0
            ...
            print(f"[{mg.contract:11}] {name:16} {metric:9}={value:.3f}{extra_str} skill={skill:+.3f} "
    ...
    json.dump([{"name": r[0], "contract": r[1], "metric": r[2], "value": r[3], "skill": r[4],
                "arch": r[5], "params": r[6], "field": r[7], "sota": r[8]} for r in rows],
              open(_out, "w"), default=float)
